...
Encoder Representations from Transformers) vyvinutý výskumníkmi v spoločnosti Google.
Moderné NLP modely vďaka kontextuálnemu spracovaniu slov poskytujú lepšie
výsledky pri porozumení textu a zvládajú zložitejšiu interpretáciu dopytov.

V posledných rokoch sa pozornosť sústreďuje na tzv. Large Language Models (\gls{llm}),
ktoré disponujú vysokou kapacitou parametrov a umožňujú, aby model dokázal
generalizovať vedomosti z obrovských textových korpusov. Tieto modely sú schopné generovať
zrozumiteľné texty a spracúvať dotazy v prirodzenom jazyku na vysokej úrovni.

V oblasti finančných aplikácií sa \gls{llm} modely osvedčujú najmä pri
spracovaní neštruktúrovaných dotazov nad štruktúrovanými dátami — napríklad pri
prevode otázky v prirodzenom jazyku na databázový dopyt (Text-to-SQL). Rothman (2023)
zdôrazňuje, že architektonický základ Transformer modelov im umožňuje porozumieť
doménovej terminológii a kontextovým väzbám, čo je predpokladom ich úspešného nasadenia
v špecializovaných systémoch, akým je FinanceGPT~\cite{rothman2023transformers}.
...
spracovania pretrvávala~\cite{lipton2015critical}.
...
schopnosti — vrátane zero-shot a few-shot učenia. Práve tieto vlastnosti sú kľúčové
pre konverzačné systémy vo finančnej doméne, kde model musí z voľného textu
spoľahlivo rozlíšiť zámer používateľa a dynamicky volať vhodné systémové
nástroje~\cite{vaswani2017attention, zhao2023survey}.
...
of Experts, napríklad pri modeloch Mixtral alebo Gemini), ktoré selektívne aktivujú
len časť neurónovej siete, čím optimalizujú prevádzkové náklady
\cite{zhao2023survey}. Sľubným smerom je aj architektúra SSM (\textit{State
Space Models}, napríklad Mamba), ktorá sľubuje lineárnu zložitosť spracovania
kontextu na rozdiel od kvadratickej zložitosti Transformerov
\cite{rothman2024transformers}.

\paragraph{Výber modelov pre produkciu a benchmarking}
Pre aplikačné nasadenie prostredníctvom API vrstvy (ako je to navrhnuté v
architektúre FinanceGPT) však vnútorná architektúra modelu nehrá primárnu rolu.
Rozhodujúcim faktorom je \textit{výstupný výkon}, latencia a schopnosť
dodržiavať štruktúrovaný formát. Výber modelov pre integráciu a následné testovanie
sa preto exaktne opiera o výsledky štandardizovaných benchmarkov:
...
\begin{itemize}
	\item \textbf{GPT-4 trieda modelov (napr. GPT-4o-mini):} Predstavuje zlatý
		...
		na komplexnom datasete Spider viac ako 86\,\% úspešnosť prevodu Text-to-SQL \cite{gao2023texttosql}.
	...
	\item \textbf{Llama 3.1 8B Instruct:} Tento model je v práci zaradený ako
		...
		štruktúrovanom výstupe, čo potvrdzuje aj Zhao et al. (2023)
		\cite{zhao2023survey}.
\end{itemize}

Okrem samotného výkonu v benchmarkoch je pri integrácii \gls{llm} do fakturačného
systému výber podmienený aj ďalším súborom špecifických kritérií, ktoré
...
voči neúplným alebo škodlivým vstupom~\cite{zhao2023survey}.
...
aj odborná literatúra~\cite{zhao2023survey,atkinson2025}.
